...
set Libero_project_name_CFG1 SF2_Adv_Dev_Kit_TS_MiV_RV32IMA_CFG1_BaseDesign

set project_folder_name_CFG2 MiV_CFG2_BD
set project_dir_CFG2 "./$project_folder_name_CFG2"
set Libero_project_name_CFG2 SF2_Adv_Dev_Kit_TS_MiV_RV32IMA_CFG2_BaseDesign
...
if {"$config" == "CFG1"} then {
	if {[file exists $project_dir_CFG1] == 1} then {
		...
		source ./import/components/IMA_CFG1/import_component_and_constraints_sf2_adv_dev_kits_rv32ima_cfg1.tcl
		save_project
        base_design_built
	}
	...
	if {[file exists $project_dir_CFG2] == 1} then {
		...
		download_cores_all_cfgs
		source ./import/components/IMA_CFG2/import_component_and_constraints_sf2_adv_dev_kits_rv32ima_cfg2.tcl
		save_project
        base_design_built
	}
} elseif {"$config" != ""} then {
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$config" == "CFG1"} then {
		export_prog_job \
			-job_file_name {SF2_Adv_Dev_Kit_TS_MiV_RV32IMA_CFG1_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {SF2_Adv_Dev_Kit_TS_MiV_RV32IMA_CFG2_BaseDesign} \
