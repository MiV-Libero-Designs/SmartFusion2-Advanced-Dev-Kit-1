...
set Libero_project_name_CFG1 SF2_Adv_Dev_Kit_TS_MiV_RV32IMC_CFG1_BaseDesign

set project_folder_name_CFG2 MiV_CFG2_BD
set project_dir_CFG2 "./$project_folder_name_CFG2"
set Libero_project_name_CFG2 SF2_Adv_Dev_Kit_TS_MiV_RV32IMC_CFG2_BaseDesign

set project_folder_name_CFG3 MiV_CFG3_BD
set project_dir_CFG3 "./$project_folder_name_CFG3"
set Libero_project_name_CFG3 SF2_Adv_Dev_Kit_TS_MiV_RV32IMC_CFG3_BaseDesign
...
{
	puts "\n---------------------------------------------------------------------------------------------------------"
	puts "Creating a new project for the 'SF2_Adv_Dev_Kit_TS' board."
	puts "--------------------------------------------------------------------------------------------------------- \n"
}
...
{
	puts "\n---------------------------------------------------------------------------------------------------------"
	puts "Error: A project exists for the 'SF2_Adv_Dev_Kit_TS' with this configuration."
	puts "--------------------------------------------------------------------------------------------------------- \n"
}
...
proc pre_configure_place_and_route {config}\
{
	if {"config" == "CFG1"} then {
		...
		# Configuring Place_and_Route tool for a timing pass CFG2 & CFG3
...
if {"$config" == "CFG1"} then {
	...
	if {[file exists $project_dir_CFG2] == 1} then {
		...
		download_cores_all_cfgs
		source ./import/components/IMC_CFG2/import_component_and_constraints_sf2_adv_dev_kits_rv32imc_cfg2.tcl
		save_project
        base_design_built
	}
	...
	if {[file exists $project_dir_CFG3] == 1} then {
		...
		create_new_project_label
		new_project -location $project_dir_CFG3 -name $Libero_project_name_CFG3 -project_description {} -block_mode 0 -standalone_peripheral_initialization 0 -instantiate_in_smartdesign 1 -ondemand_build_dh 1 -hdl {VERILOG} -family {SmartFusion2} -die {M2S150TS} -package {1152 FC} -speed {-1} -die_voltage {1.2} -part_range {COM} -adv_options {IO_DEFT_STD:LVCMOS 2.5V} -adv_options {RESTRICTPROBEPINS:1} -adv_options {RESTRICTSPIPINS:0} -adv_options {TEMPR:COM} -adv_options {UNUSED_MSS_IO_RESISTOR_PULL:None} -adv_options {VCCI_1.2_VOLTR:COM} -adv_options {VCCI_1.5_VOLTR:COM} -adv_options {VCCI_1.8_VOLTR:COM} -adv_options {VCCI_2.5_VOLTR:COM} -adv_options {VCCI_3.3_VOLTR:COM} -adv_options {VOLTR:COM} 
		download_cores_all_cfgs
		source ./import/components/IMC_CFG3/import_component_and_constraints_sf2_adv_dev_kits_rv32imc_cfg3.tcl
		save_project
        base_design_built
	}
} elseif {"$config" != ""} then {
...
configure_tool -name {SYNTHESIZE} -params {BLOCK_MODE:false} -params {BLOCK_PLACEMENT_CONFLICTS:ERROR} -params {BLOCK_ROUTING_CONFLICTS:LOCK} -params {CLOCK_ASYNC:250} -params {CLOCK_DATA:5000} -params {CLOCK_GLOBAL:2} -params {PA4_GB_COUNT:16} -params {PA4_GB_MAX_RCLKINT_INSERTION:16} -params {PA4_GB_MIN_GB_FANOUT_TO_USE_RCLKINT:300} -params {RAM_OPTIMIZED_FOR_POWER:0} -params {RETIMING:false} -params {SYNPLIFY_OPTIONS:} -params {SYNPLIFY_TCL_FILE:}
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$config" == "CFG1"} then {
		export_prog_job \
			-job_file_name {SF2_Adv_Dev_Kit_TS_MiV_RV32IMC_CFG1_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {SF2_Adv_Dev_Kit_TS_MiV_RV32IMC_CFG2_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {SF2_Adv_Dev_Kit_TS_MiV_RV32IMC_CFG3_BaseDesign} \
