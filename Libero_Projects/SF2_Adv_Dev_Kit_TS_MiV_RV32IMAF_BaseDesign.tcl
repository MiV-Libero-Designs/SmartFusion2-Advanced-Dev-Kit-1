...
set Libero_project_name_CFG1 SF2_Adv_Dev_Kit_TS_MiV_RV32IMAF_CFG1_BaseDesign
...
if {"$config" == "CFG1"} then {
	if {[file exists $project_dir_CFG1] == 1} then {
		...
		source ./import/components/IMAF_CFG1/import_component_and_constraints_sf2_adv_dev_kits_rv32imaf_cfg1.tcl
		save_project
        base_design_built
	}
} elseif {"$config" != ""} then {
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	run_tool -name {GENERATEPROGRAMMINGFILE}
		...
		export_prog_job \
			-job_file_name {SF2_Adv_Dev_Kit_TS_MiV_RV32IMAF_CFG1_BaseDesign} \
